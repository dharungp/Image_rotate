...
def image_sketch(decode_array_to_img):
    ...
    sharping_gray_img = cv2.bitwise_not(blur_img)
    sketch_img = cv2.divide(converted_gray_img,sharping_gray_img,scale=256.0)
    status,output_img = cv2.imdecode('PNG',sketch_img)
